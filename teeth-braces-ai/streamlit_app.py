...
def load_model(weights_path: str = 'best.pt'):
    ...
    # If direct path doesn't exist, check standard relative paths
    if not os.path.exists(weights_path):
        candidate_paths = [
            os.path.join(script_dir, weights_path),
            os.path.join(repo_root, weights_path),
            os.path.join(script_dir, 'best.pt'),
            os.path.join(repo_root, 'best.pt'),
            'best.pt',
            'teeth-braces-ai/best.pt',
            '../braces_dataset_fixed_yolov8/runs/detect/train_final/weights/best.pt',
            '../braces_dataset_fixed_yolov8/runs/detect/train/weights/best.pt',
            'runs/detect/train_final/weights/best.pt',
        ...
        if found_weights:
            ...
            if not os.path.exists(weights_path):
                ...
                weights_path = 'yolov8n.pt' if os.path.exists('yolov8n.pt') else 'yolov8l.pt'
...
def process_image(image: Image.Image, model, conf: float = DEFAULT_CONFIDENCE, imgsz: int = 960) -> tuple:
    ...
    if boxes is not None and len(boxes) > 0:
        for box in boxes:
            ...
            # Skip low confidence detections (use user-selected threshold)
            if conf_val < conf:
                continue
...
def main():
    ...
    with st.sidebar:
        ...
        weights_path = st.text_input(
            "Model Weights Path",
            value="best.pt",
            help="Path to the trained YOLO model weights (default: best.pt in teeth-braces-ai/)"
        )
        ...
        **Teeth Braces Detection AI**
        ...
        This application uses YOLOv8 (Nano model) to detect orthodontic 
    ...
    st.markdown(
        ...
        "🦷 Teeth Braces Detection AI | Powered by YOLOv8 (Nano Model)"
        "</p>",
        unsafe_allow_html=True
    )
